...
def graphs_tuple_to_broadcasted_sharded_graphs_tuple(
    graphs_tuple: jraphzzz.GraphsTuple, num_shards: int
...
def broadcasted_sharded_graphs_tuple_to_graphs_tuple(sharded_graphs_tuple):
    ...
    def unbroadcast(y):
        return tree.tree_map(lambda x: x[0], y)
    def unshard(x):
...
def ShardedEdgesGraphNetwork(
    ...
    aggregate_nodes_for_globals_fn: jraphzzz.AggregateNodesToGlobalsFn = jax.ops.segment_sum,
