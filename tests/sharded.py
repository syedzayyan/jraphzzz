...
import jraphzzz
import numpy as np
import jraphzzz.utils.utils as utils
from jraphzzz.data.sharded import ShardedEdgesGraphsTuple
from jraphzzz.nn.base import ShardedEdgesGraphNetwork
from jraphzzz.utils.utils import (
...
def get_graphs_tuples(n_edge, sharded_n_edge, device_graph_idx):
    ...
    def broadcast(x):
        return np.broadcast_to(x[None, :], [devices] + list(x.shape))
...
@pytest.mark.parametrize(
    "test_name, n_edge, sharded_n_edge, device_graph_idx",
    [
        ("split_3_to_4", [3, 5, 4], [[3, 3], [2, 4]], [[0, 1], [1, 2]]),
        ("split_zero_last_edge", [1, 2, 5, 4], [[1, 2, 3], [2, 4, 0]], [[0, 1, 2], [2, 3, 0]]),
        ("split_one_over_multiple", [1, 11], [[1, 3], [4, 0], [4, 0]], [[0, 1], [1, 0], [1, 0]]),
    ]
)
def test_get_sharded_graphs_tuple(setup_xla, test_name, n_edge, sharded_n_edge, device_graph_idx):
    in_tuple, expect_tuple = get_graphs_tuples(
        n_edge, sharded_n_edge, device_graph_idx
    )
    out_tuple = graphs_tuple_to_broadcasted_sharded_graphs_tuple(
...
def test_sharded_same_as_non_sharded(setup_xla,test_name, n_edge):
    in_tuple = _get_graphs_from_n_edge(n_edge)
    devices = 3
    sharded_tuple = graphs_tuple_to_broadcasted_sharded_graphs_tuple(
        in_tuple, devices
    )
