...
class NeighborFinder:
    def __init__(self, adj_list, key: jr.PRNGKey, uniform=False):
        """
        Params
        ------
        node_idx_l: List[int]
    ...
    def init_off_set(self, adj_list):
        ...
        n_idx_l = jnp.array(n_idx_l)
        n_ts_l = jnp.array(n_ts_l)
        e_idx_l = jnp.array(e_idx_l)
        off_set_l = jnp.array(off_set_l)
    ...
    def get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbors=20):
        ...
        out_ngh_node_batch = jnp.zeros((len(src_idx_l), num_neighbors)).astype(int)
        out_ngh_t_batch = jnp.zeros((len(src_idx_l), num_neighbors)).astype(jnp.float32)
        out_ngh_eidx_batch = jnp.zeros((len(src_idx_l), num_neighbors)).astype(int)
        
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ngh_idx, ngh_eidx, ngh_ts = self.find_before(int(src_idx), float(cut_time))

            if len(ngh_idx) > 0:
                if self.uniform:
                    sampled_idx = jr.randint(self.key, minval=0, maxval=len(ngh_idx), shape=(num_neighbors,))
                    
                    # Use .at[].set() for JAX array assignment
                    out_ngh_node_batch = out_ngh_node_batch.at[i, :].set(ngh_idx[sampled_idx])
                    out_ngh_t_batch = out_ngh_t_batch.at[i, :].set(ngh_ts[sampled_idx])
                    out_ngh_eidx_batch = out_ngh_eidx_batch.at[i, :].set(ngh_eidx[sampled_idx])
                    
                    # resort based on time
                    pos = out_ngh_t_batch[i, :].argsort()
                    out_ngh_node_batch = out_ngh_node_batch.at[i, :].set(out_ngh_node_batch[i, :][pos])
                    out_ngh_t_batch = out_ngh_t_batch.at[i, :].set(out_ngh_t_batch[i, :][pos])
                    out_ngh_eidx_batch = out_ngh_eidx_batch.at[i, :].set(out_ngh_eidx_batch[i, :][pos])
                else:
                    ...
                    # Use .at[].set() for JAX array assignment
                    out_ngh_node_batch = out_ngh_node_batch.at[i, num_neighbors - len(ngh_idx):].set(ngh_idx)
                    out_ngh_t_batch = out_ngh_t_batch.at[i, num_neighbors - len(ngh_ts):].set(ngh_ts)
                    out_ngh_eidx_batch = out_ngh_eidx_batch.at[i, num_neighbors - len(ngh_eidx):].set(ngh_eidx)
                    
        return out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch

    def find_k_hop(self, k: jnp.ndarray, src_idx_l: jnp.ndarray, cut_time_l: jnp.ndarray, num_neighbors: int = 20):
        """Sampling the k-hop sub graph
        """
        x, y, z = self.get_temporal_neighbor(src_idx_l, cut_time_l, self.key, num_neighbors)
        node_records = [x]
        eidx_records = [y]
        t_records = [z]
        for _ in range(k - 1):
